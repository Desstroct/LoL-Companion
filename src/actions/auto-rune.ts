...
export class AutoRune extends SingletonAction<AutoRuneSettings> {
	...
	/** Last champion key detected — avoids refetching for the same champ */
	private lastChampKey = "";
	/** Cached rune pages from Lolalytics */
	private lastRunes: RunePageData[] = [];
	/** Index of the currently selected build variant (0 = highest WR, 1 = most common) */
	private selectedIndex = 0;
	/** Whether we already applied runes for the current champion */
	...
	override onWillDisappear(_ev: WillDisappearEvent<AutoRuneSettings>): void | Promise<void> {
		this.stopPolling();
	}
	...
	override async onKeyDown(ev: KeyDownEvent<AutoRuneSettings>): Promise<void> {
		await this.applyRunes(ev.payload.settings);
	}
	...
	override async onDialRotate(ev: DialRotateEvent<AutoRuneSettings>): Promise<void> {
		if (this.lastRunes.length < 2) return;
		this.selectedIndex = (this.selectedIndex + 1) % this.lastRunes.length;
		this.applied = false;
	...
	private startPolling(): void {
		...
		this.updateState();
		this.pollInterval = setInterval(() => this.updateState(), 3000);
	}
	...
	private async updateState(): Promise<void> {
		...
		if (phase !== "ChampSelect") {
			if (this.lastChampKey !== "") {
				// Reset on leaving champ select
				this.lastChampKey = "";
				this.lastRunes = [];
		...
		if (champKey === this.lastChampKey) return; // Already fetched for this champion

		this.lastChampKey = champKey;
		this.selectedIndex = 0;
		this.applied = false;
	...
	private async renderAll(settings: AutoRuneSettings): Promise<void> {
		for (const a of this.actions) {
			const rune = this.lastRunes[this.selectedIndex];
			const champ = this.lastChampKey ? dataDragon.getChampionByKey(this.lastChampKey) : null;
			...
			const appliedMark = this.applied ? " ✅" : "";
	...
	/**
	 * Apply the selected rune page to the League Client.
	 * Strategy: find or create an editable page named RUNE_PAGE_NAME, then update it.
	 */
	private async applyRunes(settings: AutoRuneSettings): Promise<void> {
		const rune = this.lastRunes[this.selectedIndex];
		if (!rune) {
